...
pub fn build_correspondence_and_prefixes(
    ...
    let macro_studied = macros.get(macro_name).unwrap_or_else(|| {
        panic!(
            "La macro '{}' no aparece en el JSON de especificación",
            macro_name
        )
    });
...
pub fn resolve_all_macros(
    ...
    for (node_id, macro_name) in &correspondence {
        let entry = entries_by_name.get(macro_name.as_str()).unwrap_or_else(|| {
            panic!(
                "La macro '{}' (nodo {}) no se pudo analizar a partir del JSON de especificación",
                macro_name, node_id
            )
        });
...
pub fn resolve_full(
    ...
    let main_macro = macro_defs
        .get("main")
        .ok_or_else(|| "El JSON de especificación no tiene macro 'main'".to_string())?;
    let root_macro_name = find_call_target(&main_macro.formula).ok_or_else(|| {
        "No se encontró ninguna llamada '@...' en la macro 'main' del JSON de especificación".to_string()
    })?;

    let root_pos = *nodeid2pos
        .get(&0)
        .ok_or_else(|| "No hay ningún nodo con node_id 0 en la estructura".to_string())?;
    ...
    let main_entry = macros
        .iter()
        .find(|e| e.name == "main")
        .ok_or_else(|| "La macro 'main' no se pudo analizar a partir del JSON de especificación".to_string())?;
