...
fn main() {
    ...
    let (macros, macro_defs) = match read_smt_specification(&args.file) {
        ...
        Err(_) => {
            let source =
                fs::read_to_string(&args.file).expect("No se pudo leer el fichero de entrada");
    ...
    match (&args.correspondence, &args.structure, &macro_defs) {
        ...
        (Some(correspondence_path), Some(structure_path), Some(macro_defs)) => {
            let (_, name_to_signal) = read_signal_correspondence(correspondence_path)
                .expect("No se pudo leer el JSON de correspondencia de señales");
            let structure =
        ...
        _ => {
            eprintln!(
                "--correspondence y --structure solo tienen sentido juntos, y si `file` es el JSON de especificación (con vars_info)"
            );
            std::process::exit(1);
        }
    }
