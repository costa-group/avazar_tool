...
// Trait to to handle the leiden/louvain calls for the various graph backends
pub trait CanLeiden {
    fn num_edges(&self) -> usize;

    // Wrapper call for the various leiden/louvain calls. 
...
impl CanLeiden for WeightedArcs<usize> {
    ...
    fn get_partition(self: Box<Self>, resolution: f64, _max_iterations: usize, seed: Option<u64>) -> Vec<Vec<usize>> {
        ...
        let result: Vec<HashSet<usize, _>> = rs_louvain_communities(self.original_nodes, self.arcs, IdentityIndexer, true, Some(resolution * 0.25), Some(1e-6), seed).unwrap(); // fixing a bug in their implementation
