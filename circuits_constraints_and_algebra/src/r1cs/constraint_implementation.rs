...
impl Constraint for R1CSCosntraint<usize> {
    ...
    fn normalise<'a>(&'a self, prime: &'a BigInt) -> Vec<R1CSCosntraint<usize>> {
        ...
        if self.is_nonlinear() {
            choices_a = self.a.get(&0).map(|bigint| vec![Cow::Borrowed(bigint)]).unwrap_or_else(|| division_normalise( self.a.values(), prime, true ) );
            choices_b = self.b.get(&0).map(|bigint| vec![Cow::Borrowed(bigint)]).unwrap_or_else(|| division_normalise( self.b.values(), prime, true ) );
        ...
        if let Some(c_constant) = self.c.get(&0) {
        ...
        // Otherwise if there are no AB choices normalise by C division norm
            ...
            choices_c = division_normalise( self.c.values(), prime, true );
        ...
        choices.into_iter().map(|((a_factor, b_factor), c_factor)| {
            let nonlinear_part_a = self.a.keys().map(|sig| (*sig, div(self.a.get(sig).unwrap(), &a_factor, prime).ok().unwrap()) ).collect::<HashMap<usize, BigInt>>();
    ...
    fn fingerprint<'a, T: Hash + Eq + Default + Copy + Ord + Debug>(&'a self, fingerprint: &mut Option<Self::Fingerprint<'a, T>>, signal_to_fingerprint: &HashMap<usize, T>) -> () {
        ...
        if let Some(existing_fingerprint) = fingerprint.as_mut() {
            ...
            for key in self.c.keys() {new_fingerprint.entry(*key).or_insert_with(|| Characteristic::default()).2 = self.c.get(key);}

            if self.is_ordered() {
                for key in self.a.keys() {new_fingerprint.entry(*key).or_insert_with(|| Characteristic::default()).0.0 = self.a.get(key);}
                for key in self.b.keys() {new_fingerprint.entry(*key).or_insert_with(|| Characteristic::default()).1 = self.b.get(key);}
            } else {

                let (lsignals, rsignals) = (self.a.keys().copied().collect::<HashSet<_>>(), self.b.keys().copied().collect::<HashSet<_>>());
                ...
                for key in in_both.iter().copied() {
                    new_fingerprint.entry(key).or_insert_with(|| Characteristic::default()).0 = sort_pair_bigint(self.a.get(&key), self.b.get(&key));}
                for (key, bigint) in only_left.map(|sig| (sig, self.a.get(sig))).chain(only_right.map(|sig| (sig, self.b.get(sig)))) {
    ...
    fn is_nonlinear(&self) -> bool{
        self.a.len() > 0 && self.b.len() > 0
    }

    fn is_ordered(&self) -> bool {!(self.a.len() > 0 && self.b.len() > 0 && self.a.values().sorted().eq(self.b.values().sorted()))}
    fn is_bridge_constraint(&self, prime: &BigInt, strict: bool) -> bool {
        !self.is_nonlinear() && self.c.len() == 2 && self.c.values().sum::<BigInt>() == *prime && (!strict || self.c.values().any(|coef| *coef == BigInt::from(1)))
    }
...
impl ShuffleConstraint for R1CSCosntraint<usize> {
    fn add_random_constant_factor(&mut self, rng: &mut impl Rng, field: &BigInt) -> () {
        ...
        for (bigint, big_factor) in self.a.values_mut().map(|bigint| (bigint, a_bigfactor)).chain(
                                    self.b.values_mut().map(|bigint| (bigint, b_bigfactor))).chain(
                                    self.c.values_mut().map(|bigint| (bigint, c_bigfactor))) 
